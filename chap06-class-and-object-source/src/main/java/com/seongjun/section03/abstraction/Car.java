...
public class Car {
    ...
    /* 시동이 걸려있는 상태인 경우에는 할 일이 없고, 시동이 걸려 있지 않은 상태인 경우 시동을 건다. */
    public void starUp(){
        if(isOn) {
    ...
    /* 시동이 걸려이쓴ㄴ 상태인 경우 시동을 끄고, 이미 꺼진 상태라면 끌 수 없다. */
    public void turnOff(){
        if (isOn){
            if (speed>0){
                ...
                this.isOn = false;
                System.out.println("시동을 끕니다. 다시 운행하시려면 시동은 켜주세요");
            }
        } else {
            System.out.println("이미 시동이 꺼저 있는 상태입니다. 시동 상태를 확인해주세요.");
        }
    }
}
